...
function PresentDay({ date, report }) {
    ...
    const { todayFood, addTodayReport, calGoal } = useDatabase()
    ...
    useEffect(() => {
        ...
        if(data) {
            if(data.hasOwnProperty('dished') === false) {
    ...
    useEffect(() => {
        ...
        if(report) {
            ...
            if(report.hasOwnProperty('food')) {
                ...
                setBreakfastList(report.hasOwnProperty('breakfastList') ? report['breakfastList'] : [])
                setBreakfastCount(report.hasOwnProperty('breakfastCount') ? report['breakfastCount'] : [])
                setBreakfastCal(report.hasOwnProperty('breakfastCal') ? report['breakfastCal'] : temp)
    
                setLunchList(report.hasOwnProperty('lunchList') ? report['lunchList'] : [])
                setLunchCount(report.hasOwnProperty('lunchCount') ? report['lunchCount'] : [])
                setLunchCal(report.hasOwnProperty('lunchCal') ? report['lunchCal'] : temp)
    
                setDinnerList(report.hasOwnProperty('dinnerList') ? report['dinnerList'] : [])
                setDinnerCount(report.hasOwnProperty('dinnerCount') ? report['dinnerCount'] : [])
                setDinnerCal(report.hasOwnProperty('dinnerCal') ? report['dinnerCal'] : temp)
    ...
    useEffect(() => {
        ...
        if(eaten < calGoal/2) {
            setMotiText("Let's get healthier!")
        ...
        {calcul !== null && 
            <div id="day-container">
                <div className='calculation-card'>
                    <h3>{motiText}</h3>
                    {toEat > 0 && <h5>{toEat} calories to go!!!</h5> }
                ...
                <div className='food-item-card'>
                    ...
                    <Button disabled={breakfastList.length === 0 || lunchList.length === 0 || dinnerList === 0} onClick={saveChanges}>
                        Save
                    </Button>
                </div>
            </div>
